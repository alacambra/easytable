...
public class ExcelLikeExampleTest {
  ...
  private Table createSimpleExampleTable() throws IOException {

    // Some data
    String text = "Hello Monika is this potato Ok? something else";
    final Object[][] data = {
        {"Whisky", text, 145.00},
        {"Beer", 768.00, 677.00},
        {"Gin", 456.02, 612.00},
    ...
    float fWidth = HELVETICA.getAverageFontWidth() / 1000;
    float w = HELVETICA.getStringWidth(text + " €") * fonzSize;


    System.out.println(w);
    // Define the table structure first
    final TableBuilder tableBuilder = Table.builder()
        .addColumnsOfWidth(100, max, 50, 50)
        .fontSize(fonzSize)
        .font(HELVETICA)
